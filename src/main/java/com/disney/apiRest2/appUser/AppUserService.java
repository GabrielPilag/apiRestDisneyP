...
public class AppUserService implements UserDetailsService{
	...
	public String signUpUser(AppUser appUser) {
		...
		String token = UUID.randomUUID().toString();//Creo un Identificador unico universal aleatorio para generar el token
		
		ConfirmationToken confirmationToken = new ConfirmationToken(token, LocalDateTime.now(), LocalDateTime.now().plusMinutes(15), appUser); //genero el token de confirmacion
	
		confirmationTokenService.saveConfirmationToken(confirmationToken);//guardo en mi base de datos el token generado
